// features/progress/bloc/progress_cubit.dart
...
class ProgressCubit extends Cubit<ProgressState> {
  ...
  static ProgressState _calculate() {
    ...
    final learned = repo.topics.expand((t) => t.words.where((w) => w.learned)).length;
    final progress = total > 0 ? (learned.toDouble() / total.toDouble()) * 100 : 0.0; // ← toDouble()

    final topicProgress = repo.topics.map((t) {
      return t.words.isEmpty ? 0.0 : (t.words.where((w) => w.learned).length.toDouble() / t.words.length.toDouble()) * 100;
    }).toList();
